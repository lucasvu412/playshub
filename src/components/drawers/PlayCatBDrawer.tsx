...
export default function PlayCatBDrawer({ open, onClose }: PlayCatBDrawerProps) {
  ...
  const checkQuestMutation = useMutation({
    mutationFn: () => checkQuest("DAILY", "PLAY_CAT_BATTLE"),
  });
  const proceedQuestMutation = useMutation({
    mutationFn: () => proceedQuest("DAILY", "PLAY_CAT_BATTLE"),
  });
  const task = data?.find((item) => item.requestType === "PLAY_CAT_BATTLE");
  const earn = task?.reward?.match(/PLAYS:(\d+)/)?.[1] || 0;

  const play = async () => {
    await proceedQuestMutation.mutateAsync();
    const link = `https://catb.playshub.io/?tgWebAppStartParam#tgWebAppData&user={id:${user?.id},first_name:${user?.first_name},last_name:${user?.last_name}}`;
    ...
    <Drawer open={open} footer={null} placement="bottom" onClose={onClose}>
      <Flex vertical align="center" gap={15}>
        ...
        <Flex vertical align="center">
          ...
          <Typography.Text>Play Cat Battle game to get bonus</Typography.Text>
        </Flex>
