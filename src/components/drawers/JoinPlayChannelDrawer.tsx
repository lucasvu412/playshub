...
export interface JoinPlayChannelDrawerProps {
  open: boolean;
  onClose: () => void;
}

export default function JoinPlayChannelDrawer({
  open,
  onClose,
}: JoinPlayChannelDrawerProps) {
  const { data, refetch } = useQuery({
    queryKey: ["get_quest_status"],
    queryFn: getQuestStatus,
  });
  const checkQuestMutation = useMutation({
    mutationFn: () => checkQuest("DAILY", "JOIN_PLAYS_CHANNEL"),
  });
  const proceedQuestMutation = useMutation({
    mutationFn: () => proceedQuest("DAILY", "JOIN_PLAYS_CHANNEL"),
  });
  const task = data?.find((item) => item.requestType === "JOIN_PLAYS_CHANNEL");
  const earn = task?.reward?.match(/PLAYS:(\d+)/)?.[1] || 0;

  const join = async () => {
    await proceedQuestMutation.mutateAsync();
    if (task?.additional) {
      Telegram?.WebApp?.openTelegramLink(task.additional);
    }
  };
      ...
      <Flex vertical align="center" gap={15}>
        <Flex vertical align="center">
          <div style={{ padding: 10 }}>
            <Image
              src="/icons/earn/join-play-channel.png"
              preview={false}
              width={80}
            />
          </div>
        ...
        <Flex vertical align="center">
          ...
          <Typography.Text>Join PLAYS Channel to get rewards</Typography.Text>
        </Flex>
