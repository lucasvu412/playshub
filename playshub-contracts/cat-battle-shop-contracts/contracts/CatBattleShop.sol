    ...
    PausableUpgradeable
{
    ...
    /// @custom:storage-location erc7201:CatBattle.storage.CatBattleShop
    struct CatBattleShopStorage {
        mapping(uint256 => Item) items;
        uint256 nextItemId;
    }

    // keccak256(abi.encode(uint256(keccak256("CatBattle.storage.CatBattleShop")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant CatBattleShopStorageLocation =
        0xadb6f487badddc1ee7708c8641d75e23f4d61ac9e01c30b2e72a0531a5cac400;

    function _getCatBattleShopStorage()
        private
        pure
        returns (CatBattleShopStorage storage $)
    {
        assembly {
            $.slot := CatBattleShopStorageLocation
        }
    }
    ...
    function initialize(
        ...
        __CatBattleShop_init_unchained(itemNames_, itemPrices_);
        __UUPSUpgradeable_init();
        __Ownable_init_unchained(owner_);
        __Pausable_init();
    }

    function __CatBattleShop_init_unchained(
        string[] memory itemNames_,
        uint256[] memory itemPrices_
    ...
    function getItem(uint256 id) external view override returns (Item memory) {
        CatBattleShopStorage storage $ = _getCatBattleShopStorage();
