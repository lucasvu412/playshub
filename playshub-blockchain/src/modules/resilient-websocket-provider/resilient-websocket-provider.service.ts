...
@Injectable()
export class ResilientWebsocketProviderService {
  private logger = new Logger(ResilientWebsocketProviderService.name);
    ...
    return new Promise((resolve) => {
      const startConnection = () => {
        if (this.reconnectionAttempts >= MAX_RECONNECTION_ATTEMPTS) {
          console.error(
            `Max reconnection attempts (${MAX_RECONNECTION_ATTEMPTS}) reached for ${this.wsUrl}. Stopping reconnection.`,
          );
          this.terminate = true;
          resolve(null);
          return;
        ...
        this.ws.on('open', async () => {
          ...
          try {
            ...
            this.logger.error(
              `Error initializing WebSocketProvider for ${this.wsUrl}:`,
              error,
            );
        ...
        this.ws.on('close', () => {
          this.logger.error(
            `The websocket connection was closed for ${this.wsUrl}`,
          );
        ...
        this.ws.on('error', (error) => {
          this.logger.error(`WebSocket error for ${this.wsUrl}:`, error);
        });
